...
<form method='post' action='<?php echo $rootUrl?>/helpers/projectSearchbox.php'>
<input type='hidden' name='return_form' value='<?php echo $zen->ffv($return_form); ?>'>
<input type='hidden' name='return_field' value='<?php echo $zen->ffv($return_field); ?>'>
<?php if( $onechoice ) { ?>
  <input type='hidden' name='onechoice' value='1'>
<?php } ?>
<table width='<?php echo $table_width?>' align='center'>
...
<tr>
  ...
  <td class='bars'><input type='text' name='search_param[ticket_id]' size='12' 
	maxlength='10' value='<?php echo $zen->ffv($search_param["ticket_id"]); ?>'></td>
</tr>
<tr>
  <td class='bars'><?php echo tr("Title"); ?></td>
  <td class='bars'><input type='text' name='search_param[title]' 
	size='22' maxlength='20' value='<?php echo $zen->ffv($search_param["title"]); ?>'><td>
</tr>
<tr>
  <td class='bars'><?php echo tr("Description"); ?></td>
  <td class='bars'><input type='text' name='search_param[description]' 
	size='22' maxlength='20' value='<?php echo $zen->ffv($search_param["description"]); ?>'></td>
</tr>
<tr>
...
<script language='javascript'>
   ...
   function saveValues() {
     ...
     for(i=0; i<document.helperForm.elements.length; i++) {
       element = document.helperForm.elements[i];
       if( element.type == "<?php echo $onechoice? 'radio' : 'checkbox'?>" 
     ...
     <?php echo $field?>.value = val;
     if( <?php echo $field?>.select ) {
       <?php echo $field?>.select();
     }
     else if( <?php echo $field?>.focus ) {
       <?php echo $field?>.focus();
     }
     if( <?php echo $field?>.onchange ) {
       <?php echo $field?>.onchange();
     }
...
<form action='<?php echo $rootUrl?>/helpers/projectSearchbox.php' method='post' name='helperForm'>
<?php
...
<tr>
  <td class='bars' colspan='4' align='center'>
     <input type='button' 
      class='submit' value='<?php echo tr("Select"); ?>' <?php echo (is_array($results))?
