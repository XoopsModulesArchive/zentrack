...
?>
  <LINK HREF="<?php echo $rootUrl?>/styles.php" REL="STYLESHEET" TYPE="text/css">
  <script type="text/javascript">
    var imageUrl = <?php echo $zen->fixJsVal($imageUrl); ?>;
    var rootUrl = <?php echo $zen->fixJsVal($rootUrl); ?>;
    var id = <?php echo $zen->fixJsVal(id); ?>;
    var hotkeyHelpDelay = <?php echo $zen->fixJsVal($zen->getSetting('hotkeys_help_delay')); ?>;
    var hotkeyHintDelay = <?php echo $zen->fixJsVal($zen->getSetting('hotkeys_hint_delay')); ?>;
    var loaded = false;
  </script>
  <script language="javascript" src="<?php echo $rootUrl?>/javascript.js"></script>
  <script language="javascript" src="<?php echo $rootUrl?>/keyevent.js"></script>
  <script type='text/javascript'>
    var isIE = navigator.appName.indexOf('Microsoft') >= 0;
    var debugOn = <?php echo $Debug_Mode > 0? 'true' : 'false'?>;
...
<script type='text/javascript'>
	...
	var zenSubPath = '<?php echo $ZENTRACK_MODULE_SUB?>'; 		//eg.: '/zentrack/modules'
...
<a href="<?php echo $rootUrl?>"><img
  src="<?php echo $imageUrl?>/zenTrack_logo.jpg"
  ...
  <td class='indent' valign='bottom' align='left'><table cellpadding='0' cellspacing='0' border='0'>
    <tr><?php echo  $nav_tabs ?></tr></table></td>
  <td align='right'>
     <form action="<?php echo $rootUrl?>/quickSearch.php" name='quickIdForm'>
      <?php echo $hotkeys->ll("Search"); ?>:</span>&nbsp;
      <input class="searchbox" type="text" name="idText"
        size="15" maxlength="255" onfocus='openHelpBox(window.document.getElementById("searchBoxHelp"),this)' title="<?php echo $hotkeys->tt("Search"); ?>">
        &nbsp;(<a class='small' href='<?php echo $rootUrl?>/search.php' title='<?php echo $hotkeys->tt('Advanced Search'); ?>'>
          <?php echo $hotkeys->ll('Advanced Search', 'Advanced'); ?></a>)&nbsp;&nbsp;
       <div id='searchBoxHelp' style='width:110px;height:75px;display:none;' class='helpBox'>
       <?php echo tr("Enter a ticket ID or text to search for"); ?></div>
    </form>
  </td>
  <td width='125'>
    <div class='small' align='center'><b><?php echo $zen->showLongDate(); ?></b></div>
    <div class='small' align='center'><?php echo tr("version"); ?>
      <?php echo $zen->getSetting("version_xx"); ?>
      <?php if( $zen->demo_mode == 'on' ) { print "<div class='small' align='center'>(demo mode)</div>"; } ?>
    </div>
  </td>
  </tr>
  <tr>
    <td width='100%' colspan='<?php echo $nav_col_span?>' class='titleCell' height='<?php echo $nav_bar_height?>' align='right'><img
      src='<?php echo $imageUrl?>/empty.gif' width='930' height='1'></td>
  </tr>
  <tr>
  <td valign="top" height="400" colspan='<?php echo $nav_col_span?>' class='mainContent navpad'><?php
