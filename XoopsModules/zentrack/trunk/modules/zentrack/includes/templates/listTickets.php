...
<?php
  ...
  if( $show_totals ) {
    ?>
      <td width="40" height="25" valign="middle" title="<?php echo tr("Percent completed"); ?>" class='headerCell'>
        <?php echo tr("%"); ?>
      </td>
    <?php    
  }
...
<!--- BEGIN Paging --->
<tr>
   <td  align="right" valign='bottom' colspan='<?php echo $cols?>' class='subTitle'>
     <?php
...
?>
   <tr>
       <td colspan="<?php echo  $cols ?>" class="subTitle">
       <nobr>
       <form method="post" action="search.php" name='searchModifyForm' style="display: inline; margin: 0px;">
          ...
          <input type="hidden" name="search_text" value="<?php echo $zen->ffv($search_text); ?>">
          <?php
     ...
     <form method="post" action="exportSearch.php" style="display: inline; margin: 0px;">
          <input type="submit" class="smallSubmit" value="<?php echo tr("Export Results"); ?>">
          <input type="hidden" name="search_text" value="<?php echo $zen->ffv($search_text); ?>">
          <?php
          if( is_array($search_params) ) {
           foreach($search_params as $k=>$v) {
