...
<?php if( $type == "type" ) { ?>
      <p><?php echo tr('There are two special types, which can be set by including either the word <span class="error">project</span> or <span class="error">note</span> in the bin name.'); ?></p>

     <p><?php echo tr('Projects act as containers, which can hold multiple tickets as "tasks" which are part of the projects completion requirements.'); ?></p>
     <p><?php echo tr('Notes are special tickets which do not require any actions.').'  '.
  ...
  <script type='text/javascript'>
    function checkPriType( formObj ) {
      ...
      if( !matched ) {
        return confirm("<?php echo tr("You have not set a 'Project' type, projects will not work correctly.  Choose 'cancel' to try again."); ?>");
      }
      return true;
    }
  </script>
<?php } ?>
      <ul>
      <form name='configForm' action='<?php echo $SCRIPT_NAME?>' <?php echo $type=='type'? " onsubmit='return checkPriType(this)'":""?> method='post'>
     ...
     ?>
	   <b><?php echo tr("Edit the Active ?",array($plural_type)); ?></b>
	 </td>
	 </tr>
	 <tr toofar="toofar">
	 <td width="30" class='cell' align='center'><b><?php echo tr("ID"); ?></b></td>
	 <td class='cell' align='center'><b><?php echo tr("Name"); ?></b></td>
	 <td width="30" class='cell' align='center'><b><?php echo tr("Active"); ?></b></td>
...
<tr id='aSampleRow' style="display:none;">
  <td class='bars'><?php echo $newtext?></td>
  <input type='hidden' name='newID[<?php echo $sampleRowId?>]' value=''>
  <td class='bars'><input type='text' name='newName[<?php echo $sampleRowId?>]' value='' size='20' maxlength='25'></td>
  <td class='bars'><input type='checkbox' name='newActive[<?php echo $sampleRowId?>]' value='1' checked></td>
  <td class='bars'>
    <input type='hidden' name='newPri[<?php echo $sampleRowId?>]' value='<?php echo $sampleRowId?>'>
    <a href='#' onClick='moveRowUp(this.parentNode);return false;'
      border='0' alt='Move Up' title='Move Up'><img src='<?php echo $rootUrl?>/images/icon_arrow_up.gif'
      width='16' height='16' alt='Move Up' title='Move Up' border='0'></a>
    <a href='#' onClick='moveRowDown(this.parentNode);return false;' 
      border='0' alt='Move Down' title='Move Down'><img src='<?php echo $rootUrl?>/images/icon_arrow_down.gif'
      width='16' height='16' alt='Move Down' title='Move Down' border='0'></a>
    <a href='#' onClick='addRow(this);return false;'
      border='0' alt='Add Section' title='Add Section'><img src='<?php echo $rootUrl?>/images/icon_add.gif'
      width='16' height='16' alt='Add Section' title='Add Section' border='0'></a>
  </td>
</tr>    
<tr toofar="toofar" id="submitRow">
  <td class="titleCell" colspan="5">
    <?php echo tr("Press Save to save changes"); ?>
    <br>
    <?php echo tr("Press Reset to return to original values"); ?>
  </td>
</tr>
      <tr toofar="toofar">
	 <td class='cell' colspan='5'>
         <input type='hidden' name='TODO' value=''>
	 <input type='submit' class='submit' value='<?php echo uptr("Save"); ?>' onClick='return setToDo("Save")'>
	 &nbsp;
	 <input type='submit' class='submitPlain' value='<?php echo uptr("Reset"); ?>' onClick='return setToDo("Reset")'>
         </td>
      </tr>
      </table>
      </ul>
...
var i;
var ci;
var rowCount = <?php echo $sampleRowId+1?>;
    ...
    if( sect.hasChildNodes() && sect.childNodes.length > 0 ) {
      for(var j=0; j < sect.childNodes.length; j++) {
        ...
        if( c.type == 'text' || c.type == 'checkbox' || c.type == 'hidden' ) {
          c.setAttribute('name', c.getAttribute('name').replace('<?php echo $sampleRowId?>',newName));
