...
?>
<tr>
  <td class='<?php echo ($tf_type && !$tf_data)?"titleCell":"subTitle"?>' colspan='3'>
    <?php echo tr("Data Selection"); ?>
  </td>
</tr>
<?php
  if( $tf_type ) {     
...
?>
<form method='post' action='<?php echo $rootUrl?>/reports/custom.php' name='reportDataForm'>
<input type='hidden' name='report_type' value='<?php echo $zen->ffv($report_type); ?>'>
<tr>
  <td class="bars">
    <?php echo tr("Select ?s",array($zen->ffv($report_type))); ?>
  </td>
  <td class="bars">
<?php
   if( $searchbox ) {
