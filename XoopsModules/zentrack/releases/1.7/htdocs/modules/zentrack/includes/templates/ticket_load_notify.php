...
?>
  ...
  if( is_array($notify_list) ) {
    ?>
    <form name='dropNotifyForm' action="<?php echo $rootUrl?>/actions/dropFromNotify.php" method="post">
    ...
    <td class='headerCell'><?php echo tr("Email"); ?></td>
    <?php if($drop) {?> <td class='headerCell'><?php echo tr("Delete"); ?></td> <?php } ?>
    </tr>
    <?php  
    ...
    <?php if( $add ) { ?>
      ...
      <form name='notifyAddForm' action="<?php echo $rootUrl?>/actions/addToNotify.php">
      <input type="hidden" name="id" value="<?php echo $zen->checkNum($id); ?>">
      <input type="hidden" name="setmode" value="<?php echo $zen->ffv($page_mode); ?>">
