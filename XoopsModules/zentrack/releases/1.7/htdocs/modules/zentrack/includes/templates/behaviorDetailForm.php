...
?>
      ...
      ?>
      <ul>
      <form name='behaviorDetailForm' action='<?php echo $SCRIPT_NAME?>' onsubmit='return checkForValueColumn(this)' method='post'>
      <input type='hidden' name='TODO' value=''>
      <input type='hidden' name='behavior_id' value='<?php echo $zen->checkNum($behavior_id); ?>'>
      <table cellpadding="2" cellspacing="1" class='plainCell'>
	 <tr>
	 <td class='titleCell' align='center' colspan='<?php echo $colspan?>'>
	   <b>Edit the Behavior Rules</b>
	 </td>
	 </tr>
	 <tr>
	 <td class='cell' align='center'><b><?php echo tr("Compare Field"); ?></b></td>

   <?php if( !$groupIsFile ) { ?>
	 <td class='cell' align='center'><b><?php echo tr("Operator"); ?></b></td>
   <?php } ?>
	 
   <td class='cell' align='center'><b><?php echo 
      $groupIsFile? tr('Column Number') : tr("Compare Value"); ?></b></td>

   <?php if( !$groupIsFile ) { ?>
	 <td class='cell' align='center'><b><?php echo tr("Sort Order"); ?></b></td>
   <?php } ?>
    ...
    ?>
<tr>
  <td class="titleCell" colspan="<?php echo $colspan?>">
    ...
    <?php echo tr('Press Reset to return to original values'); ?>
      ...
      <tr>
	 <td class='cell' colspan='<?php echo $colspan?>'>
	 ...
	 &nbsp;
	 <input type='submit' class='submit' value='<?php echo tr('Save'); ?>' onClick="setTodo('Save')">
	 &nbsp;
   ...
   function checkForValueColumn(formObj) {
     ...
     alert("<?php echo tr("You must supply a value_column before this behavior will function."); ?>");
     return false;
   }
      </script> 
